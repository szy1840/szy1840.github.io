...
另外注意，第三层使用 `offset` 动画而非条件渲染（`if isDrawerVisible`），性能更好。同时，通过 `.allowsHitTesting(isDrawerVisible)` 控制交互性，避免隐藏时拦截事件。从设计思路来讲，Drawer 并不是显示/隐藏两种状态下的两个不同视图，它始终存在于视图层级中，只是通过位移控制可见性，通过 hit-testing 控制交互性。
...
首先，排除可能：ZStack 内的后两个层只是 overlay（覆盖层），并不影响第一层 ScrollView 的 layout。特别是注意到，侧边栏层是使用 offset 实现的，出现/隐藏不影响布局，只影响渲染位置，因此也不会触发 re-calculation。这两层是不可能引发页面跳动的。
