...
date: 2025-10-27
tags: ["iOS", "Firebase", "Social", "Architecture", "Feed System"]
lang: zh
...
这里的主要区别是，如果 moments 放在 `users/{authorId}/moments` 这种用户下的子集合里，那么 firebase 自动生成的 ID 只在该作者的子集合范围内不重复，并不一定全局唯一，导致 feed 里面也无法简单地使用 momentId 作为 Id，必须额外存信息，比如 authorId，变成复合索引结构。
...
不过，由于 Firestore 有每个 batch 最大 500 写操作的限制，因此如果一个用户发布动态特别多，需要做分页/游标清理。
