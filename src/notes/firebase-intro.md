...
Firebase 是 Google 提供的 BaaS 平台，非常适合独立开发者、全栈工程师或小团队快速构建 Web、iOS、Android 应用。它集成了数据库、身份验证、托管、函数、存储等核心能力，让开发者几乎不需要传统后端就能构建完整的应用。
...
初始化时选择所需模块（常用的包括 Hosting、Firestore、Functions）。项目结构通常如下：
...
Firebase 依靠 Firestore Rules 实现细粒度访问控制。一个最小安全规则示例：
